...
std::pair<EggResult, CheckerResult> Eggecutor::RunInteractive(const Path& source, const Path& checker,
                                                              ...
                                                              const std::string& fixed_interaction) const {
    Path stdin = os.TmpFile();
    Path stdout = os.TmpFile();
    Path stderr = os.TmpFile();
    ...
    if (fixed_interaction.size()) {
        os.RunBashCommand(StrCat("rm ", stdout));
        os.RunBashCommand(StrCat("touch ", stdout));
        os.WriteFile(stdout, fixed_interaction);
        run_command = StrCat(
                "/bin/true & ",
                checker_lang->RunCommand(checker), " <", stdout, " 2>", checker_stderr, " ; wait $! ");
    } else {
        run_command =
            StrCat(language->RunCommand(source), " ", "<", stdin, " ", "1>", stdout, " ", "2>", stderr, " & ",
...
bool ProgrammingLanguage::CPP::Compile(const Path& source) {
    ...
    auto err = os.TmpFile();
    auto result = Eggecutor::ExecuteCommand("g++ -std=c++14 -O2 -Wall -DTUMBLETEST " + source + " -o " + this->BinaryFile(source) +
                                            " 2>" + err);
