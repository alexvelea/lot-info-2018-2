...
def cpp_compile(source):
    ...
    try:
        subprocess.check_output(["g++", "-O2", "-Wall", "-std=c++11", "./sources/" + source + ".cpp", "./main.cpp", "-o", "./sources/" + source, "-lm", "-DLOCAL_RUN", "-Wl,-stack_size,0xd00000000"], stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        print("Compile error:")
        print(e.output)
    print("Done")
